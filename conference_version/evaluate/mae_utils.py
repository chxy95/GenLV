...
import models_mae, models_mae_lyh, models_mae_vqgan, models_mae_vqgan2
...
def prepare_model_retrained2(chkpt_dir, arch='mae_vit_large_patch16_dec512d8b_input448', img_size=448, device='cpu', strict=True):
    # build model
    model = getattr(models_mae_vqgan2, arch)(img_size=img_size)
    # load model
...
def prepare_model_retrained(chkpt_dir, arch='mae_vit_large_patch16_dec512d8b_input448', img_size=448, device='cpu', strict=True):
    # build model
    model = getattr(models_mae_vqgan, arch)(img_size=img_size)
    # load model
    checkpoint = torch.load(chkpt_dir, map_location='cpu')
    msg = model.load_state_dict(checkpoint['model'], strict=strict)
    print(msg)
