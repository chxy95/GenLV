...
def gen_tone_map(img, w_group=0):
    ...
    J = lut_normalized[(255 * img).astype(np.int32)]
    # smooth:
...
def gen_pencil_drawing(img, kernel_size, stroke_width=0, num_of_directions=8, smooth_kernel="gauss",
    ...
    if not pencil_texture_path:
        pencil_texture = io.imread('/cpfs01/user/puyuandong/glv/DiffBIR/DiffBIR_NRIQA/pencil0.jpg', as_gray=True)
    else:
...
if __name__ == "__main__":
    input_folder = "data/MIT-fivek/expert_C_test"
    output_folder = "data/MIT-fivek/expert_C_Pencil_GT_test"
    ...
    for i in tqdm(range(len(input_list))):
        input_path = os.path.join(input_folder, input_list[i])
        img = cv2.imread(input_path)
        pencil_drawing = gen_pencil_drawing(img, kernel_size=8, stroke_width=1, num_of_directions=8, smooth_kernel="gauss",
                        gradient_method=1, rgb=True, w_group=2, stroke_darkness=2, tone_darkness=1.5)
        output_path = os.path.join(output_folder, input_list[i])
        cv2.imwrite(output_path, pencil_drawing*255)
    
    # img = cv2.imread('/cpfs01/user/puyuandong/glv/Real-ESRGAN/datasets/BSRGAN_degradation/urban100_1/hq/0.png')
    # pencil_drawing = gen_pencil_drawing(img, kernel_size=8, stroke_width=1, num_of_directions=8, smooth_kernel="gauss",
    #                     gradient_method=1, rgb=True, w_group=2, stroke_darkness=2, tone_darkness=1.5)
    # cv2.imwrite('inputs/icl/lq/pencil.png', pencil_drawing*255)
