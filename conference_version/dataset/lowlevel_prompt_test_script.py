...
# if __name__ == "__main__":
    # img_gt = cv2.imread('./test.png')
    # x_distortion_dict = distortions_dict
    # deg_type = random.choice(x_distortion_dict['blur'])
    # print(deg_type)
    # img_lq = globals()[deg_type](img_gt, 5)
    # cv2.imwrite('output.png', img_lq)
...
class DatasetPrompt_Train(Dataset):
    def __init__(self, dataset_path, input_size, ITS_path=None, Rain13K_path=None,
                 ...
                 Style_Raphael_path=None, data_len=None, tasks_version=None
                 ):
    ...
    def __getitem__(self, idx):
        ...
        if dataset_choice == 'Base':
            ...
            tasks_flag = np.random.choice(tasks_list, p=[9/20, 9/20, 1/10])
...
class DatasetPrompt_Val(Dataset):
    def __init__(self, dataset_path, input_size=320, data_len=None, tasks_version=None):
        self.paths_gt, self.sizes_gt = util.get_image_paths('img', dataset_path)
