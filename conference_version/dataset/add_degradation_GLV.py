...
from dataset.basicsr.utils.img_util import img2tensor, tensor2img
from dataset.basicsr.utils.diffjpeg import DiffJPEG
from dataset.basicsr.utils.img_process_util import filter2D
from torch.nn import functional as F
from dataset.basicsr.utils.degradations import circular_lowpass_kernel, random_mixed_kernels,random_add_gaussian_noise_pt
import torch
...
def add_degra(img,degradation,param):
    if degradation == 'rain':
        # rain
        value = np.random.randint(50, 200)  # 雨线数目
        #value = 75  # 雨线数目
        img = add_rain(img, value)
        param += 'rain:{} * '.format(value)
    if degradation == 'haze':
        # haze
        ...
        param += 'haze:{} {} * '.format(A, B)
    if degradation == 'snow':
        # snow
        # A = round(random.uniform(0.8, 0.95), 2)
        # B = round(random.uniform(0.5, 1.0), 2)
        A = 0.9
        B = 0.75
        img = add_snow(img, depth_path, snow_mask_path, A, B)
        param += 'snow:{} {} * '.format(A, B)
    ...
    if degradation == 'blur':
        ...
        # sth = round(random.uniform(0.2, 3), 2)
        ks = 15
        sth = random.uniform(1, 4)
        img = blur_tensor(img, ks, sth)
        param += 'blur:{} {} * '.format(ks, sth)

    if degradation == 'noise':
        # noise
        sth = round(random.uniform(1, 30))
        #sth = 20
        img = random_add_gaussian_noise_pt(
            img, sigma_range=[sth, sth], clip=True, rounds=False,
            gray_prob=0)
        param += 'noise:{} * '.format(sth)
    if degradation == 'jpeg':
        # jpeg
        sth = round(random.uniform(20, 95))
        #sth = 50
        img = convertToJpeg(img, sth)
        param += 'jpeg:{} * '.format(sth)

    if degradation == 'ringing':
        # sinc
        # kernel_range = [2 * v + 1 for v in range(3, 11)]
        # ks = random.choice(kernel_range)
    ...
    if degradation == 'r-l':
        ...
        img = r_l(img)
        param += 'r-l * '
    if degradation == 'inpainting':
        # inpainting
        # l_num = random.randint(5, 10)
        # l_thick = random.randint(5, 10)
        l_num = random.randint(10, 10)
        l_thick = random.randint(7, 7)
        img = inpainting(img, l_num, l_thick)
        param += 'inpainting * '
...
if __name__ == '__main__':
    ...
    # 进出全是cv2，需要pil的话在函数里面转。
    degree = 1
    for degree in range(1,6):


        GT_folder = '/home/xtkong/GLV/GLV-benchmark/image/original/GT'

        #save_GT_folder = '/home/xtkong/GLV/GLV-benchmark/GLV-data/single/deblur/GT'
        save_LR_folder = '/home/xtkong/GLV/GLV-benchmark/image/mix/d{}/LQ'.format(degree)
        img_GT_list = sorted(os.listdir(GT_folder))
        snow_mask_list = sorted(os.listdir('/home/xtkong/GLV/GLV-benchmark/image/single/desnow/mask'))


        for path_GT in img_GT_list[0:1]:
            img_name=path_GT.split('.')[0]
            log_path = os.path.join(save_LR_folder, img_name) + '.txt'
            save_LR_folder_=os.path.join(save_LR_folder,img_name)

            print(path_GT)

            depth_path='/home/xtkong/GLV/GLV-benchmark/image/original/depth/LR_tensor/{}/predict_depth.mat'.format(
            img_name)
            index = random.randint(0, 99)
            snow_mask_path = os.path.join('/home/xtkong/GLV/GLV-benchmark/image/single/desnow/mask', snow_mask_list[index])

            img = cv2.imread(os.path.join(GT_folder, path_GT))
            ...
            w = w - (w % 4)
            img = img[0:w, 0:h, :]
            # img_GT = img_GT[0:w, 0:h, :]

            param=''

            d_l1=['SR', 'blur', 'noise', 'jpeg', 'inpainting', 'ringing', 'r-l','rain','haze','snow']
            d_l2=['SR', 'blur', 'noise', 'jpeg', 'inpainting', 'ringing', 'r-l']
            ...
            if degree==1:
                ...
                for d1 in d_l1:
                    param0 = ''
                    img1, param1 = add_degra(img, d1, param0)

                    img_num += 1

                    check_dir(save_LR_folder_)
                    with open(log_path, "a+") as f:
                        f.write(img_name + '_' + str(img_num).rjust(6, "0") + ': ' + param1 + '\n')
                    cv2.imwrite(
                        os.path.join(save_LR_folder_, img_name + '_' + str(img_num).rjust(6, "0") + '.png'),img1)
            ...
            if degree==2:
                ...
                for d1 in d_l1:
                    ...
                    img1, param1 = add_degra(img, d1, param0)
                    for d2 in d_l2:
                        img2, param2 = add_degra(img1, d2, param1)

                        img_num += 1

                        check_dir(save_LR_folder_)
                        with open(log_path, "a+") as f:
                            f.write(img_name + '_' + str(img_num).rjust(6, "0") + ': ' + param2 + '\n')
                        cv2.imwrite(
                            os.path.join(save_LR_folder_, img_name + '_' + str(img_num).rjust(6, "0") + '.png'),
                            img2)
            if degree==3:
                print(degree)
                img_num = 0
                for d1 in d_l1:
                    ...
                    for d2 in d_l2:
                        img2, param2 = add_degra(img1, d2, param1)
                        for d3 in d_l2:
                            img3, param3 = add_degra(img2, d3, param2)

                            img_num += 1

                            check_dir(save_LR_folder_)
                            with open(log_path, "a+") as f:
                                f.write(img_name + '_' + str(img_num).rjust(6, "0") + ': ' + param3 + '\n')
                            cv2.imwrite(
                                os.path.join(save_LR_folder_, img_name + '_' + str(img_num).rjust(6, "0") + '.png'),
                                img3)
            if degree==4:
                ...
                for d1 in d_l1:
                    ...
                    for d2 in d_l2:
                        ...
                        for d3 in d_l2:
                            img3, param3 = add_degra(img2, d3, param2)
                            for d4 in d_l2:
                                img4, param4 = add_degra(img3, d4, param3)
                                ...
                                check_dir(save_LR_folder_)
                                with open(log_path, "a+") as f:
                                    f.write(img_name + '_' + str(img_num).rjust(6, "0") + ': ' + param4 + '\n')
                                cv2.imwrite(
                                    os.path.join(save_LR_folder_, img_name + '_' + str(img_num).rjust(6, "0") + '.png'),
                                    img4)
                    ...
                    for d2 in d_l2:
                        ...
                        for d3 in d_l2:
                            ...
                            for d4 in d_l2:
                                img4, param4 = add_degra(img3, d4, param3)
                                for d5 in d_l2:
                                    img5, param5 = add_degra(img4, d5, param4)
                                    img_num += 1

                                    check_dir(save_LR_folder_)
                                    with open(log_path, "a+") as f:
                                        f.write(img_name + '_' + str(img_num).rjust(6, "0") + ': ' + param5 + '\n')
                                    cv2.imwrite(os.path.join(save_LR_folder_, img_name+'_'+str(img_num).rjust(6, "0")+'.png'), img5)
