...
from dataset.add_flare import add_flare
...
class DatasetPrompt_Train(Dataset):
    def __init__(self, dataset_path, input_size, ITS_path=None, Demoireing_path=None,
                 Desnowing_path=None, FlareRemoval_path=None, Rain13K_path=None, DustRemoval_path=None,
                 FaceSR_path=None, HighlightRemoval_path=None, LOL_path=None, ReflectionRemoval_path=None,
                 ShadowRemoval_ISTD_path=None, ShadowRemoval_ISTD_adjusted_path=None, ShadowRemoval_SRD_path=None,
                 TextSR_train1_path=None, TextSR_train2_path=None, Backlit_path=None,
                 ExposureError_path=None, InstagramFilter_path=None, SDR_HDR_path=None, 
                 LLF_path=None, FiveK_path=None, RedEye_path=None, UDC_poled_path=None, 
                 UDC_toled_path=None, Bokeh_path=None, Histo_Equ_path=None, 
        ...
        # Red Eye Removal dataset
        # Pass
        if RedEye_path is not None:
            self.dataset_path_gt_RedEye = os.path.join(RedEye_path, 'gt')
            self.dataset_path_lq_RedEye = os.path.join(RedEye_path, 'lq')
            self.paths_gt_RedEye, self.sizes_gt_RedEye = util.get_image_paths('img', self.dataset_path_gt_RedEye)
            self.paths_lq_RedEye, self.sizes_lq_RedEye = util.get_image_paths('img', self.dataset_path_lq_RedEye)
            self.paths_RedEye_len = len(self.paths_lq_RedEye)
            sorted(self.paths_gt_RedEye)
            sorted(self.paths_lq_RedEye)
        ...
        if Bokeh_path is not None:
            self.dataset_path_gt_Bokeh = os.path.join(Bokeh_path, 'original')
            self.dataset_path_lq_Bokeh = os.path.join(Bokeh_path, 'bokeh')
    ...
    def __getitem__(self, idx):
        ...
        # need further check: why one-the-fly
            ...
            if light_source_type == 'Synthetic':
                light_source_path = self.paths_light_synthesis[random.randint(0, len(self.paths_light_synthesis)-1)]
            elif light_source_type == 'Real':
                light_source_path = self.paths_light_real[random.randint(0, len(self.paths_light_real)-1)]
            else:
            ...
            img_lq1 = add_flare(img_gt1.copy(), img_flare, img_relaticve, img_light_source)
            img_lq2 = add_flare(img_gt2.copy(), img_flare, img_relaticve, img_light_source)
        ...
        elif dataset_choice == 'RedEye_Removal':
            # Unknown split of dataset
            pass
        ...
        # WhiteBalance is replaced by Base. Some Problems have not been solved.
            ...
            lq2_name = '_'.join(gt2_path.split('/')[-1].split('_')[:-2])
            lq2_path = os.path.join(self.dataset_path_lq_WhiteBalance, '{}_{}'.format(lq2_name, wb_suffix))
