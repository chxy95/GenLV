...
class Single_Base_Encoder_x3(nn.Module):
    def __init__(self, in_channel=3, prompt_dim=512, nf=64): # nf=64
        super(Single_Base_Encoder_x3, self).__init__()

        self.conv_first = nn.Conv2d(in_channel, nf, 3, 1, 1)
        self.conv_hr = nn.Conv2d(nf, nf, 3, 1, 1) # ori * ori * 64
        self.norm = nn.LayerNorm(nf)

        self.fea_level1 = make_layer(ResidualBlockNoBN, 4, num_feat=nf)
        self.pixel_unshuffle1 = nn.PixelUnshuffle(2) # ori//2 * ori//2 * 64*4
        self.channel_reduction1 = nn.Conv2d(4 * nf, 2 * nf, 1) # ori//2 * ori//2 * 64*2
        self.norm1 = nn.LayerNorm(2 * nf)

        self.fea_level2 = make_layer(ResidualBlockNoBN, 4, num_feat=2 * nf)
        self.pixel_unshuffle2 = nn.PixelUnshuffle(2) # ori//4 * ori//4 * 64*2*4
        self.channel_reduction2 = nn.Conv2d(4 * 2 * nf, 4 * nf, 1) # ori//4 * ori//4 * 64*4
        self.norm2 = nn.LayerNorm(4 * nf)

        self.fea_level3 = make_layer(ResidualBlockNoBN, 4, num_feat=4 * nf)
        self.pixel_unshuffle3 = nn.PixelUnshuffle(2) # ori//8 * ori//8 * 64*4*4
        self.channel_reduction3 = nn.Conv2d(4 * 4 * nf, 8 * nf, 1) # ori//4 * ori//4 * 64*8
...
class XRestormer_Prompt_SpatCrossAttn(nn.Module):
    def __init__(self, 
        ...
        # self.Prompt_KV_Encoder_2 = Prompt_Unified_KV_Encoder_x3(prompt_dim=int(dim*2**3))
    ...
    def forward(self, imgs, visual_tokens=None, input_is_list=False, mask_ratio=None):
        ...
        # prompt_lq, prompt_gt = self.Prompt_KV_Encoder_2(prompt_input, prompt_target)
...
# nf=32: FLOPs=201.58G, Params=32.70M
