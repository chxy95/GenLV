    ...
    for root, dirs, files in os.walk(dir_path):
        for filename in files:
            if filename.lower().endswith('.png'):
                png_filelist.append(osp.join(root, filename))
    return png_filelist

data_path = '../data/Common528_plus/target_256x256'
img_path_list = get_filelist(data_path)
# print(img_path_list)
...
#     img_save_path = osp.join('../data/Common528_plus', deg_type)
#     os.makedirs(img_save_path, exist_ok=True)
#     for img_path in img_path_list:
#         img = util.read_img(None, img_path, None, float=True)
#         img_out, _ = add_degradation_one_image(img, deg_type)
#         out_path = osp.join(img_save_path, osp.basename(img_path))
...
#     os.makedirs(img_save_path, exist_ok=True)
#     for img_path in img_path_list:
#         img = util.read_img(None, img_path, None, float=False)
#         img_out, _ = calculate_operators_one_image(img, deg_type)
#         out_path = osp.join(img_save_path, osp.basename(img_path))
#         cv2.imwrite(out_path, img_out)

# -------------------- onthefly_degradation_list3 --------------------
# def add_x_distortion_1(img_gt, deg_type):
#     # np.uint8, BGR
...
#     for img_path in img_path_list:
#         img = util.read_img(None, img_path, None, float=False)
#         img_out, _, severity = add_x_distortion_1(img, deg_type)
        
#         img_name, img_ext = osp.basename(img_path).split('.')
#         out_path = osp.join(img_save_path, '_'.join([img_name, str(severity)])+ '.' + img_ext)
#         cv2.imwrite(out_path, img_out)

# -------------------- onthefly_degradation_list4 --------------------
# def add_x_distortion_2(img_gt, deg_type):
#     # np.uint8, BGR
#     severity = random.choice([1, 2, 3, 4, 5])
#     if deg_type == 'noise_gaussian':
#         deg_type = random.choice(['noise_gaussian_RGB', 'noise_gaussian_YCrCb'])
#     elif deg_type == 'brightness_brighten':
#         deg_type = random.choice(distortions_dict['brighten'])
#     elif deg_type == 'brightness_darken':
#         deg_type = random.choice(distortions_dict['darken'])
#     else: deg_type = random.choice(distortions_dict[deg_type])
...
#     img_save_path = osp.join('../data/Common528_plus', task_type)
#     os.makedirs(img_save_path, exist_ok=True)
#     for img_path in img_path_list:
#         img = util.read_img(None, img_path, None, float=False)
#         img_out, _, deg_type, severity = add_x_distortion_2(img, task_type)
        
#         img_name, img_ext = osp.basename(img_path).split('.')
#         out_path = osp.join(img_save_path, '_'.join([img_name, deg_type, str(severity)])+ '.' + img_ext)
#         cv2.imwrite(out_path, img_out)


# -------------------- onthefly_OOD_blur_gaussian_lensmask --------------------
# def add_x_distortion(img_gt, deg_type):
#     # np.uint8, BGR
#     severity = random.choice([1, 2, 3, 4, 5])
...
# for img_path in img_path_list:
#     img = util.read_img(None, img_path, None, float=False)
#     img_out, _, severity = add_x_distortion(img, deg_type)
...
# def add_ood_gaussian_noise(img):
#     # np.uint8, BGR
#     # sigma = [0.05, 0.1, 0.15, 0.2, 0.25]
#     sigma = random.choice([0.025, 0.075, 0.125, 0.175, 0.225, 0.275, 0.35, 0.50])
...
# for img_path in img_path_list:
#     img = util.read_img(None, img_path, None, float=True)
#     img_out, sigma = add_ood_gaussian_noise(img)
    
#     img_name, img_ext = osp.basename(img_path).split('.')
#     out_path = osp.join(img_save_path, '_'.join([img_name, str(sigma)])+ '.' + img_ext)
#     cv2.imwrite(out_path, img_out)

# -------------------- onthefly_OOD_blur_gaussian --------------------
def add_ood_blur_gaussian(img):
    # np.uint8, BGR
    # sigma = [1, 2, 3, 4, 5]
    sigma = random.choice([0.5, 1.5, 2.5, 3.5, 4.5, 5.5])
...
for img_path in img_path_list:
    img = util.read_img(None, img_path, None, float=False)
    img_out, sigma = add_ood_blur_gaussian(img)
